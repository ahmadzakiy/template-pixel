            ...
            <mp-box v-if="!isShowDatePicker" width="412px">
              <mp-popover-list>
                <mp-popover-list-item is-disabled>
                  <mp-text color="gray.400" font-size="sm" letter-spacing="0.24em">TIME RANGE</mp-text>
                </mp-popover-list-item>
                <mp-popover-list-item
                  :is-active="selectionTimeRange === 'Today'"
                  @click="handleSelectTimeRange('Today')"
                >
                  <mp-text>Today</mp-text>
                  <mp-text> {{ formatDate(new Date()) }} </mp-text>
                </mp-popover-list-item>
                <mp-popover-list-item
                  :is-active="selectionTimeRange === 'Last 7 days'"
                  @click="handleSelectTimeRange('Last 7 days')"
                >
                  <mp-text> Last 7 Days </mp-text>
                  <mp-text>
                    {{ formatDate(getDate(-7)) }} - {{ formatDate(new Date()) }}
                  </mp-text>
                </mp-popover-list-item>
                <!-- <mp-popover-list-item
                  :is-active="selectionTimeRange === 'Last 30 days'"
                  @click="handleSelectTimeRange('Last 30 days')"
                >
                  <mp-text> Last 30 Days </mp-text>
                  <mp-text>
                    {{ formatDate(getDate(-30)) }} -
                    {{ formatDate(new Date()) }}
                  </mp-text>
                </mp-popover-list-item> -->
              </mp-popover-list>
              ...
              <mp-popover-list>
                <mp-popover-list-item
                  :is-active="selectionTimeRange === 'Per day'"
                  @click="handleSelectTimeRange('Per day')"
                >
                  Per day
                </mp-popover-list-item>
                <mp-popover-list-item
                  :is-active="selectionTimeRange === 'Per week'"
                  @click="handleSelectTimeRange('Per week')"
                >
                  Per week
                </mp-popover-list-item>
                <!-- <mp-popover-list-item
                  :is-active="selectionTimeRange === 'Per month'"
                  @click="handleSelectTimeRange('Per month')"
                >
                  Per month
                </mp-popover-list-item>
                <mp-popover-list-item
                ...
                <mp-popover-list-item
                  :is-active="selectionTimeRange === 'Custom'"
                  @click="handleSelectTimeRange('Custom')"
                >
                  Custom
                </mp-popover-list-item>
              </mp-popover-list>
...
export default {
    ...
    getFormattedValue() {
      if (this.value) {
        ...
        if (this.activeTimeRange === "Last 7 days") {
          ...
          return `Last 7 days (${this.formatDate(from)} - ${this.formatDate(
            to
          )})`;
        }
        ...
        if (this.activeTimeRange === "Last 30 days") {
          ...
          return `Last 30 days (${this.formatDate(from)} - ${this.formatDate(
            to
          )})`;
        }

        if (this.activeTimeRange === "Per day")
          return `Per day (${this.formatDate(this.value)})`;

        if (this.activeTimeRange === "Per week") {
          const getDateRange = this.getWeekDates(
            this.getWeekNumber(this.value),
        ...
        if (this.activeTimeRange === "Per month") {
          ...
          return `Per month (${this.formatDate(from)} - ${this.formatDate(
            this.getLastDateInMonth(to)
          )})`;
        }
    ...
    getSelectionInformation() {
      if (this.selectionValue) {
        ...
        if (this.selectionTimeRange === "Per week") {
          const getDateRange = this.getWeekDates(
            this.getWeekNumber(this.selectionValue),
            this.selectionValue.getFullYear()
          );
          const from = getDateRange[0];
          const to = getDateRange[1];

          return `${this.formatDate(from, false)} - ${this.formatDate(
            to,
            false
          )}`;
        }
  ...
  methods: {
    ...
    handleSelectTimeRange(timeRange) {
      ...
      if (this.selectionTimeRange === "Last 7 days")
        this.handleSelectShortcut(-7);
      if (this.selectionTimeRange === "Last 30 days")
        this.handleSelectShortcut(-30);
    ...
    getLastDateInMonth(value) {
      ...
      const lastDate = new Date(
        `${selected.getFullYear()}-${selected.getMonth() + 1}-${lastDay}`
      );
    ...
    getWeekNumber(date) {
      ...
      const days = Math.floor(
        (selectedDate - startDateOfYear) / (24 * 60 * 60 * 1000)
      );
